 ...
 * 圣遗物评分规则更新器
 *
 * 只从 miao-plugin 拉取会随角色版本变化的评分规则数据：角色词条权重、基础属性、
 * 元素、武器补正与角色特殊规则，并重新生成 utils/artis-score-standalone.js。
 *
 * 稳定展示逻辑不要写进生成模板，例如单条得分拆分、有效/无效标色、词条数估算等；
 * 这些逻辑应维护在 utils/artis-score-utils.js，避免每次更新 miao 规则时被爬虫影响。
 *
...
async function main() {
  console.log('========================================')
  console.log('  Artifact Score Rule Updater')
  console.log(`  Repo: yoimiya-kokomi/miao-plugin@${BRANCH}`)
  console.log('========================================\n')

  // ---- Phase 1: Core data files ----
  console.log('[1/5] Fetching miao rule data files...')
  ...
  try {
    ...
    console.log('  Display helpers remain external: utils/artis-score-utils.js')
  } catch (e) {
