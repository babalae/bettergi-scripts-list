...
2. 点击运行即启动脚本，当游戏中进入演奏画面时，将自动弹出UI。通过隐藏按钮（眼睛图形）可以暂时隐藏UI，从其他界面再次切换到演奏画面时，UI将自动恢复显示。
3. 曲谱文件保存在 `assets/score/` 文件夹中，如果此文件夹中的内容发生变动，需要重新运行脚本后才能在列表中看到变化后的文件。
...
## 更新日志
v1.0.3 新增UI界面，不再需要通过配置组中的设置选择曲谱。支持播放/暂停、隐藏UI、退出按钮。支持曲谱搜索。不再支持半音修正。
