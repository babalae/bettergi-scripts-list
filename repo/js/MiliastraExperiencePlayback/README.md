...
- 不要在禁止联机的区域或秘境（例如银月之庭等）运行此脚本，否则将无法运行。
- 每次通关获得 **20** 点基础经验值，根据成就完成数量的不同，每个成就可额外获得 **50** 点经验值，删除关卡存档后可重复获得成就。
- 如果开启 `删除关卡存档` 功能， **建议提前清理无用存档** ，保持存档列表简洁，减少翻页时间和误差、提升查找效率。同时修改 `每次通关获取的经验值`（例如内置的通关流程是获得5个成就，因此修改为270），缩短任务运行时间。
- 若内置的通关流程失效，请使用程序的 **[录制回放]** 功能，发挥主观能动性，自行录制 **进入关卡后的通关流程**。
- 录制完成后，打开录制回放所在目录，将录制的脚本拷贝到本脚本目录下的 `assets/playbacks` 文件夹中（建议重命名录制好的回放文件）。
...
- 领取每日奖励不会领取含有部件礼箱类型的奖励，不会帮你做选择（例如纪游只会领取经验奖励，不会领取巧趣醒转奖励）。
...
| dailyEnabled           | 执行每日通关任务                 |                                                                    | false                                                            |
| dailyRooms             | 每日奇域关卡关键词或关卡GUID     | 逗号分隔                                                           | 24429042323,28644538672,45694132064                              |
| dailyPlaybacks         | 每日通关回放文件随机池           | 同一关卡逗号分隔，不同关卡分号分隔，自行录制拷贝到assets/playbacks | 通关回放1.json,通关回放2.json;30秒按1通关.json;轰鸣木桩结算.json |
| dailyRewards           | 领取每日奖励                     | 尝试进行领取纪游经验、日活奖励，某些活动不是常驻的                 | ["诸界纪游", "绮衣珍赏", "奇趣盛邀", "星境彩馈"]                 |
| dailyLimit             | 每日单个关卡通关上限             |                                                                    | 1                                                                |
| dailyForce             | 忽略每日单个关卡通关上限         |                                                                    | false                                                            |
| closeStageDialog       | 执行通关回放前关闭游戏说明对话框 | 如果打算录制手工关闭对话框的操作，则禁用                           | true                                                             |
| goToTeyvat             | 完成后返回提瓦特大陆             |                                                                    | true                                                             |

### ❗ 注意事项

- 请确保游戏窗口分辨率比例为 `16:9`，游戏内调整分辨率后需要重启程序。
- 脚本运行期间，请保持较为良好的网络环境。
