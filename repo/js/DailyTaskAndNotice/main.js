...
const BlackConfirmButtonRo = RecognitionObject.TemplateMatch(file.ReadImageMatSync("Assets/RecognitionObject/Black Confirm Button.png"), 500, 700, 900, 360);
const BlackRedeployButtonRo = RecognitionObject.TemplateMatch(file.ReadImageMatSync("Assets/RecognitionObject/Black Redeploy Button.png"), 500, 960, 900, 100);
...
const PaimonMenuIconRo = RecognitionObject.TemplateMatch(file.ReadImageMatSync("Assets/RecognitionObject/Paimon Menu Icon.png"), 0, 0, 150, 150);
...
async function claimAdventurersGuildTasks(uname, uid) {
    ...
    if (settings.expeditionEnabled) {
        ...
        if (ExpeditionIcon.isExist()) {
            ...
            if (ExpeditionClaimButton.isExist()) {
                ...
                let RedeployButton = ro3.find(BlackRedeployButtonRo);
                ro3.dispose();
                if (RedeployButton.isExist()) {
                    log.info("重新进行派遣");
    ...
    if (settings.dailyCommissionEnabled) {
        ...
        let ClaimDailyCommissionRewardIcon = ro4.find(ClaimDailyCommissionRewardIconRo);
        ro4.dispose();
        if (ClaimDailyCommissionRewardIcon.isExist()) {
            ...
            let EncounterPointsConfirmButton = ro5.find(BlackConfirmButtonRo);
            ro5.dispose();
            ...
            let PaimonMenuIcon = ro6.find(PaimonMenuIconRo);
            ro6.dispose();
            if (PaimonMenuIcon.isExist()) {
                msg = "「每日委托」奖励未完成或已领取";
            } else {
                msg = "已领取「每日委托」奖励";
            ...
            let HideUIButton = ro7.find(HideUIButtonRo);
            ro7.dispose();
...
async function claimBattlePassRewards(uname, uid) {
    async function newBattlePassProcess() {
        ...
        let GnosticHymnButton = ro1.find(GnosticHymnButtonRo);
        let SelectedGnosticHymnButton = ro1.find(SelectedGnosticHymnButtonRo);
        ro1.dispose();
        if (CloseButton.isExist() && (GnosticHymnButton.isExist() || SelectedGnosticHymnButton.isExist())) {
            return;
        }
        for (let i=0; i<=10; i++) {
            click(1798, 45);
