    ...
    */
        ...
        for (let i = 0; i < positionSettings.length; i++) {
            ...
            while (pageTries < 20) {
                ...
                if (characterFound) {
                    ...
                    else if ((aliases[settings.roleName1] || settings.roleName1).includes(selectedCharacter)) {
                    ...
                    switch (index) {
                        ...
                        case 5:
                            ...
                            if (!!BossName && result.success) {
                                ...
                                CarryoutTask[idx].requiredBossMaterialCount = result.result;
                            } else {
                                log.info(`材料采集--存在空字符`);
                                if (!!BossName) {
                                    CarryoutTask[idx].requiredBossMaterialBossName = BossName;
    ...
    async function StatisticsRequiredMaterial(idx) {
        ...
        for (let index = 0; index < array.length; index++) {
            ...
            await sleep(500);
            log.info(`索引 ${idx + 1} 对应的元素: ${array[idx]}`);
    ...
    async function PerformOperation(carryoutTask) {
        ...
        //保底优先执行体力任务
        if (!carryoutTask[0].isMoraLeylineFlower && !carryoutTask[0].isExpBookLeylineFlower && !carryoutTask[1].requiredBossMaterialBossName && carryoutTask[1].requiredTalentBookName == "无" && !carryoutTask[2].requiredBossMaterialBossName && carryoutTask[2].requiredTalentBookName == "无") {
                ...
                await AutoFontaineLeyLine(
                    {
                        ...
                        nh: settings.nh, // 选填：好感队伍，如设定领奖前切换，同时战斗队伍必填
                        times: "4", // 选填：按刷取次数，默认6次，最多99次，树脂耗尽模式下无效
                        shuv: "1-次数刷取", // 默认：<1> 为按上面的次数刷取，可选 <2> 为耗尽树脂模式
                        color: "1-蓝花(经验花)", // 默认：<1-蓝花(经验花)>，可选<2-黄花(摩拉花)>
                        Rewardsuse: "1/2/5", // 树脂设定：1=浓缩/2=40原粹/3=脆弱/4=须臾/5=20原粹\n用`/`隔开：填写对应的树脂<数字>即可\n默认：1/2/5 (先用浓缩后原粹，直至用完，不填的不使用 ↓↓)
                        primogemUseCount: "0", // 原石购买体力次数(0~6次)，上面设置的树脂用完后才会使用 ↓↓
                        Fightquick: true, // 默认开启：异步检测战斗结束，即地脉花长出后马上停止战斗\n开启后，请关闭配置组中的<自动检测战斗结束>选项\n不启用：使用传统<打开队伍界面>判断战斗结束 ↓↓
        ...
        for (let i = 1; i < 3; i++) {
            ...
            // 读取文件内容
            log.info('准备读取文件: lib/json_relative_paths.txt');
            const txtContent = await readTxtToArray("lib/json_relative_paths.txt");
            ...
            if (!!carryoutTask[i].requiredMonsterName) {
                ...
                const targetPath1 = "敌人与魔物/" + carryoutTask[i].requiredMonsterName;
                ...
                for (let index = 0; index < result1.length; index++) {
                    ...
                    try {
                        ...
                        // 返回主界面
                        await genshin.returnMainUi();
                        await pathingScript.runFileFromUser(result1[index] + "");
            ...
            if (!!carryoutTask[i].requiredSpecialtyMaterialName && !!carryoutTask[i].requiredSpecialtyMaterialArea) {
                ...
                const targetPath2 = "地方特产/" + carryoutTask[i].requiredSpecialtyMaterialArea + "/" + carryoutTask[i].requiredSpecialtyMaterialName;
                ...
                for (let index = 0; index < result2.length; index++) {
                    ...
                    try {
                        ...
                        // 返回主界面
                        await genshin.returnMainUi();
                        await pathingScript.runFileFromUser(result2[index] + "");
    ...
    if ((aliases[settings.roleName1] || settings.roleName1) === "" && (aliases[settings.roleName2] || settings.roleName2) === "") throw new Error('未填入养成角色，脚本退出');
    ...
    if (!settings.isSkip) {
        ...
        if (OCRtargettext && OCRtargettext.includes("培养计划角色列表")) {
            ...
            // 从后往前遍历，避免删除元素后索引错乱
            if (ccr[0]) {
                newrole = newrole.filter(item => item !== ccr[0]);
            }
            if (ccr[1]) {
                newrole = newrole.filter(item => item !== ccr[1]);
            }
        } else {
        ...
        if (settings.autoFoster) {
            CarryoutTask[1].character = roleNameArray[0];
            CarryoutTask[2].character = roleNameArray[1];
        }
