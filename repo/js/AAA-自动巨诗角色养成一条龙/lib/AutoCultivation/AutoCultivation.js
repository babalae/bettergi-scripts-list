async function AutoCultivation (settings) {
    //获取BOSS材料数量
    async function getBossMaterialCount(bossName) {
        ...
        log.info(`正在查询数量`);
        try {
            const targetImageRo = RecognitionObject.TemplateMatch(file.ReadImageMatSync(`lib/AutoCultivation/assets/boss/${bossName}.png`), 0, 0, 1920, 1080);
            targetImageRo.Threshold = 0.95;
            const stopImageRo = RecognitionObject.TemplateMatch(file.ReadImageMatSync("lib/AutoCultivation/assets/boss/无相之风.png"), 0, 0, 1920, 1080);
            ...
            await sleep(800);
            const result = await findImageAndOCR("lib/AutoCultivation/assets/itemQuantityDetection.png", 200, 50, 0, 0);
            if (result !== false) {
    ...
    async function findAndClickWithScroll(targetRo, stopRo, options = {}) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            for (let i = 0; i < scrollNum; i++) {
                await keyMouseScript.runFile("lib/AutoCultivation/assets/滚轮下滑.json");
            }
    ...
    const autoNavigateToReward = async () => {
        // 定义识别对象
        const boxIconRo = RecognitionObject.TemplateMatch(file.ReadImageMatSync("lib/AutoCultivation/assets/box.png"));
    ...
    /**
     * 等待图片出现并点击
     * @param {string} imageName 图片名称（不带.png后缀且在lib/AutoCultivation/assets文件中）
    ...
    const waitAndClickImage = async (
        ...
        const imagePath = `lib/AutoCultivation/assets/${imageName}.png`;
        ...
        try {
            ...
            for (let col = 0; col < 3; col++) {
                ...
                if (result !== false) {
                    ...
                    click(clickX, clickY);
                    await sleep(1500);
                    const retryResult = await findImageAndOCR("lib/AutoCultivation/assets/itemQuantityDetection.png", 200, 50, 0, 0);
            ...
            if (afterStamina >= 40) {
                try {
                    ...
                    if (res > 0) {
                        ...
                        await pathingScript.runFile(`lib/AutoCultivation/assets/goToBoss/${bossName}前往.json`);
                        await sleep(1000);
                        log.info(`开始战斗`);
                        try {
