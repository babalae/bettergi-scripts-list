...
async function find_and_wait_fast(robj, init_wait, repeat_wait = 250, max_wait = 15000) {
    //draw_obj(robj); await sleep(2000);
    await sleep(init_wait);
    const start = Date.now();
    do {
        let result = captureGameRegion().Find(robj);
        if (result.isExist()) {
            return result;
        }
        await sleep(repeat_wait);
    } while (Date.now() - start < max_wait);
    return null;
}

const UID_REGION = template_ocr(0.87, 0.972, 0.11, 0.028, false);

/// 不传参则触发一次截图，即：get_uid() 等价于 get_uid(captureGameRegion())
function get_uid(screenshot) {
    if (screenshot == undefined) screenshot = captureGameRegion();
    let ocr = screenshot.Find(UID_REGION);
    let match = /UID\s*:\s*(\d+)/.exec(ocr.text);
    if (match) {
        return Number(match[1]);
    } else {
        logw("UID failed: " + ocr.text);
        return -1;
    }
}

function get_uid_3(screenshot) {
