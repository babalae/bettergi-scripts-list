/**
 * 根据星期配置判断是否执行脚本
 * 使用前需确保 settings.execute_Week 为数组格式（如：["星期一","星期三"]）
 * 若用户未设定或配置为空，则视为不符合执行条件
 * 00:00~04:00 视为前一天，避免跨日判断问题
 * @returns {boolean} 是否符合执行条件
 */
function shouldRunByWeekConfig() {
    const weekDays = ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"];
    const userSelection = Array.from(settings.execute_Week || []);

    // 无设定或空数组时不执行
    if (!Array.isArray(userSelection) || userSelection.length === 0) {
        log.error("星期配置为空，视为不符合执行条件");
        return false;
    }
    ...
    // 检查是否在允许的星期范围内
    const shouldRun = userSelection.includes(currentChineseDay);
    log.info(`今天为 ${currentChineseDay}，${shouldRun ? "符合" : "不符合"}执行条件`);
    return shouldRun;
}

(async function () {
    // 检查是否应该执行
    if (!shouldRunByWeekConfig()) {
        return;
    }
    // ------------------------------------
