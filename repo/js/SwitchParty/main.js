...
(async function () {
    ...
    switch (Load) {
        ...
        case LoadType.bgi_tools:
            ...
            const result = await BgiTools.getTeam({uid: uid, type: settings.type}, settings.bgi_tools_uid_team_api, token)
            Log.info(`result: {uidTeam}`, result)
            //{id,uid,team,type}
            const {team} = result
            team_name = team
            break;
    ...
    // 切换队伍
    if (team_name) {
        Log.info(`切换至队伍 {team}`, team_name)
        try {
            Log.info(`正在尝试切换至{team}`, team_name)
            if (!await genshin.switchParty(team_name)) {
                Log.info("切换队伍失败，前往七天神像重试")
                await genshin.tpToStatueOfTheSeven()
                await genshin.switchParty(team_name)
            }
        } catch {
