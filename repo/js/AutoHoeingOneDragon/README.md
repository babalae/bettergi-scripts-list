...
3. 在配置组界面中右键js，选择 **修改js脚本自定义配置** ，参考后续介绍按需修改各个配置选项
   ...
   - 如果你需要分组执行，请先建立和组数对应的配置组，分别添加本js，路径组一要【排除】的标签填写需要完全禁用的标签，如蕈兽，路径组二要【选择】的标签填写需要分配到路径组二的路线的标签，如小怪，不同配置组的js中选择对应的配队和路径组编号，其他配置保持默认的情况下即可实现精英和小怪分队伍和配置组锄地，更多路径组数量以此类推，各组其他选项原则上应保持完全相同。有更多疑问加入qq群1057307824讨论。（别再一个组填400-0，另外一个组填0-2000了）
...
### 1.9.3（2025.10.20）
1. 更新路线信息，移除index文件
2. 新增其他拾取材料的图片，需要拾取的可以自行加入到预留文件夹中
### 1.8.8（2025.10.19）
1. 修几条路线
2. 523重做
### 1.9.2（2025.10.13）
1. 修正图片文件名，横行霸者的利齿.png
### 1.9.1（2025.10.13）
1. 实装预告中的更新：记录每条路线的拾取物列表（最新不重复20个），匹配时优先处理，路线运行完成后再一次性更新至记录中
### 1.9.0（2025.10.12）
1. 更新部分小怪路线信息（149条），准备移除index文件
2. 优化路线时间自动优化相关逻辑，修正覆盖关系
3. 新增匹配阈值按照文件夹名字处理，狗粮改为0.8权重，默认改为0.85权重，文件夹名包含括号括起来的数字时，将尝试将其中图片的阈值改为该数字
4. 实装预告中的更新：bgi原版拾取也支持滚轮，滚轮间隔强制为160ms
### 1.8.7（2025.10.12）
1. 修几条路线
### 1.8.6（2025.10.11）
1. 修正settings.json中拾取模式的default项
2. 限制条件过多导致怪物数量不足或部分怪物超出目标太多时进行警告
3. 修改路径文件夹名，防止优先关键词填400后全选精英文件夹
4. 移除莉莱
...
1. 修正莫佬乱填的路线信息
2. 新增523,524路线,
3. 调整了一些路线,移除路线里落单的200怪,提升些许效率
4. 调整高效文件夹怪数+收尾=415只,多余的发配至低效
### 1.8.3（2025.10.03）
1. 添加一个try-catch来修复占位文件无法读取的问题
### 1.8.2（2025.10.03）
1. 移除ocr拾取模式，优化模板匹配拾取模式，减少误判，优化识别效率，并支持自动添加黑名单
2. 优化readme.md，更详细描述分组运行的配置方法
3. 修改默认拾取参数
4. 为沙暴路线添加沙暴标签
### 1.7.1（2025.09.27）
