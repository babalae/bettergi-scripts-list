...
7. **战斗策略选择**：
   - `使用BGI自动战斗策略(指定策略无效)`：使用BetterGI自动战斗的策略配置
   - `指定战斗策略`：可自定义战斗策略，放置在BetterGI\User\AutoFight目录下，如果没有二级目录填写文件名即可，不需要包含txt后缀如`好友分享\四神队(进阶版)`
   - API模式下选择自动时使用BGI幽境危战战斗配置的策略

8. **指定战斗策略名称**：
   - 当选择`指定战斗策略`时，填写具体的策略文件

9. **使用BetterGI原生API**：
   - 默认关闭，开启后使用BetterGI原生API执行幽境危战
   - API模式不支持功能：树脂优先级排序、挑战次数、战斗超时时间、原石购买体力
   - 开启后以下选项生效：
     - 树脂刷取次数
     - 结束后自动分解圣遗物
...
2. 新增API模式开关,运行更快，但是有功能缺失

- **优化**：
1. 完善战斗执行逻辑，增加异常处理
2. 同步最新圣遗物列表，新增「天之美赐 / 影中沉凝的幻灭」「晨星与月的晓歌 / 风起之日」圣遗物支持
