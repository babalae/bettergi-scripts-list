import { reportEffiency, queryEffiency } from "./effiency.js"
...
async function switchCardTeam(Name, shareCode) {
    ...
    async function selectTargetTeam(targetTeam) {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (res.text.trim() === targetTeam) {
                log.info("切换至队伍: {text}", res.text);
                res.click();
                await sleep(500);
                break;
            } else {
                ocrResults.push(res.text.trim());
            }
        }
 ...
 */
async function reportEfficiencyIfEnabled(charName, strategyName, success, duration, maxRetries = 3) {
    // 1. 条件检查：未启用云端统计 或 duration 为 0 时直接跳过
    if (!settings.enableCloudStatistic || !duration) {
        return;
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            log.warn("向云端汇报统计数据失败 (第{0}/{1}次尝试): {2}", attempt, maxRetries, err);
            
            if (attempt < maxRetries) {
                // 重试前稍作等待（例如 1 秒），避免连续快速重试
                await sleep(1000);
            } else {
                log.error("向云端汇报统计数据失败，已达最大重试次数: {0}", err);
            }
        }
    }
}
...
async function searchCharAndPlayCards() {
    ...
    if (settings.useFallbackStrategy && settings.enableCloudStatistic) {
        ...
        try {
            ...
            // 仅保留存在于 fallbackStrategyList 中的策略
            ...
            if (err.message.includes("JS HTTP权限")) {
                log.error("请在调度器通用设置中启用JS HTTP权限");
            } else {
                log.error("查询云端策略失败，回退本地逻辑: {0}", err);
            }
        }

        // 1) 记录所有匹配到的云端策略名
        cloudStrategies.forEach(s => cloudStrategySet.add(s));
        sortedStrategy = [...cloudStrategies];

        // 2) 插入默认策略（如果在云端策略中没有出现）
        if (!cloudStrategySet.has(DEFAULT_STRATEGY)) {
            sortedStrategy.push(DEFAULT_STRATEGY);
        }

        // 3) 过滤出未被云端命中的 fallbackStrategyList，打乱追加到末尾
        // （因为 DEFAULT_STRATEGY 不在 fallbackStrategyList 中，这里 filter 不会漏掉或重复它）
        const remainingStrategies = fallbackStrategyList.filter(s => !cloudStrategySet.has(s));
        sortedStrategy.push(...shuffleArray(remainingStrategies));

        log.info("{0}获取到云端策略{1}个，Fallback本地策略{2}个，共{3}个策略", 
            charName, cloudStrategySet.size, sortedStrategy.length - cloudStrategySet.size, sortedStrategy.length);
    } else {
    ...
    for (const strategyName of sortedStrategy) {
        ...
        // 判断当前是在使用云端策略还是本地 Fallback 策略
        const isCloud = settings.enableCloudStatistic && cloudStrategySet.has(strategyName);
        const strategyTypeTag = isCloud ? "云端推荐策略" : "Fallback本地策略";
        ...
        log.info("使用[{0}]{1}与{2}对战", strategyTypeTag, strategyName, charName);
