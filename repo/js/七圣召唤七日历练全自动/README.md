...
- 若开启了`自动尝试备用策略`，
  - 若开启了`基于云端统计数据选择策略`，则基于云端统计数据选择策略。
  - 若未开启，则按照本地策略运行记录，按成功率从大到小的顺序逐个尝试备用策略。
...
| 临时牌组 | 填写用来导入分享码的牌组名 |
| 自动尝试备用策略 | 打牌失败时自动尝试备用策略（即`牌组策略`文件夹中编号的那些） |
| 基于云端统计数据选择策略 | 在开启`自动尝试备用策略`的条件下，是否基于云端胜率和耗时的数据，决定备用策略的尝试顺序。<br>此功能需要开启脚本`通用配置`中的`JS HTTP`权限。<br>开启此设置后，也会向云端报告每个策略的胜负情况和用时。 |
| 仅使用胜率记录大于该值的备用策略 | 使用本地策略时，根据运行记录，跳过执行那些胜率太小的备用策略 |
| 密码 | 随便填 |
...
启用此选项后，每次对战结束后会向云端报告本次打牌的对手、策略名、输赢、耗时。不会上传你的UID或者角色名等游戏信息。

具体代码在`effiency.js`文件的`reportEffiency`函数中。

**关于初始数据：**

截止此脚本更新时，我的两个号总共记录了与54个角色的220场对局（打不过的可能会轮换很多个策略，所以场数比角色数多）。虽然早期的数据没有记录耗时，但是胜率也能用于辅助决策。这些就是云端的初始数据。
