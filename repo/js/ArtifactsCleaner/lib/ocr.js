...
/**
 * 等待特定图标（图片）出现
 * @param {any} targetIcon - 目标图标
 * @param {number} [timeout=5000] - 超时时间（毫秒）
 * @param {number} [retryInterval=500] - 重试间隔（毫秒）
 * @returns {Promise<Object>} - 返回包含成功状态和坐标的结果
 */
async function waitForImageAppear(targetIcon, timeout = 5000, retryInterval = 50) {
    const startTime = Date.now();

    while (Date.now() - startTime < timeout) {
        const res = await withCapture(async (region) => {
            return await region.find(targetIcon);
        });
        if (res.isExist()) {
            const elapsedTime = Date.now() - startTime;
            return { success: true, x: res.x, y: res.y, elapsed: elapsedTime };
        }
        await sleep(retryInterval);
    }
