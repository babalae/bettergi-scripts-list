...
**若脚本有问题，可[点击此处进行反馈](https://github.com/babalae/bettergi-scripts-list/issues/new?template=bug_report.yml&script-name=CD-Aware-AutoGather:2.7.0&additional-info=保留此行以便通知作者:%20@Patrick-Ze%0A%0A---%0A)**

# 使用前准备

需要你已经进行过地图追踪订阅。建议在脚本仓库的`地图追踪`中直接点击`地方特产`、`矿物`、`食材与炼金`右边的`订阅`按钮订阅类别下所有路线。

（今后的版本不再需要创建符号链接，也不再与`自动更新已订阅的脚本`功能存在协作问题）

# 运行模式

## 1. 扫描文件夹更新可选材料列表

扫描你已订阅的地图追踪任务中`地方特产`、`矿物`、`食材与炼金`三大类下的材料，并更新脚本可用的配置菜单。
...
| 物品采集数量目标 | 填写目标数量，留空表示不限制。也可以设置为`csv`，将按csv文件的配置详细设定不同材料的目标，具体见后文。`一条龙`等特殊类型会忽略此配置项 |
...
采集任务可能用到的元素共有`火水雷风`4种，此外还有挖矿类以及纳西妲两个类型，可以考虑建立两支队伍`矿纳水雷`和`矿纳火风`，即可满足所有采集任务的需要。如果你有莉奈娅，推荐让她作为挖矿位角色。
