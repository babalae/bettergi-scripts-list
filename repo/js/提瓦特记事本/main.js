...
async function recognizeWishResourceValue(templatePath, valueOffsetX, valueWidth) {
    ...
    if (recognized) {
        ...
        if (recognizedText1 && recognizedText2) {
            ...
            log.info(`提瓦特记事本-原石-${recognizedText1}`);
            log.info(`提瓦特记事本-纠缠之缘-${recognizedText2}`);
            log.info(`提瓦特记事本-抽卡资源总价值-${parseInt(recognizedText2, 10) * 160 + parseInt(recognizedText1, 10)}`);
            log.info(`提瓦特记事本-可用抽数-${parseInt(recognizedText2, 10) + (parseInt(recognizedText1, 10) / 160) | 0}`);
            if (settings.notify !== false) {
                if (!isNaN(recognizedText1) && recognizedText1.trim() !== "" && !isNaN(recognizedText2) && recognizedText2.trim() !== "") {
                    const availablePulls = parseInt(recognizedText2, 10) + (parseInt(recognizedText1, 10) / 160) | 0;
                    sendNotebookNotification(
                        `原石-${recognizedText1} | 纠缠之缘-${recognizedText2} | 可用抽数-${availablePulls}`
                    );
                } else {
                    log.warn("抽卡资源总价值转换失败");
                }
            }

            // 写入本地文件
            const filePath = unifiedRecordPath;
            const primogems = parseInt(recognizedText1, 10);
            const intertwinedFate = parseInt(recognizedText2, 10);
            const wishTotalValue = intertwinedFate * 160 + primogems;
            const availablePulls = intertwinedFate + (primogems / 160) | 0;
            const records = [];
            ...
            if (records.length > 0) {
                const result = file.WriteTextSync(filePath, `${records.join("\n")}\n`, true);
                if (result) {
...
async function runCharacterExpBookStats() {
    ...
    async function recognizeBookCount(book) {
        ...
        for (let attempt = 0; attempt < 10; attempt++) {
            ...
            try {
                ...
                if (result && result.isExist()) {
                    const countResult = gameRegion.find(
                        RecognitionObject.ocr(
                    ...
                    const digits = countResult && countResult.text
                        ? countResult.text.replace(/\D/g, "")
                        : "";
                    if (!digits) {
                        throw new Error(`${book.name}图标已找到，但数量识别失败`);
                    }
        ...
        log.info(`背包中未找到${book.name}，按0本计算`);
        return 0;
    }
