...
为了方便管理Boss配置，项目提供了一个本地HTML编辑器：`BOSS配置编辑器.html`，双击即可在浏览器中打开使用。
...
*   **选择配置文件**：手动找到并选择脚本目录下的 `assets\config\config.json` 文件
*   **拖放文件**：直接将 `config.json` 文件拖入编辑器窗口
*   **新建配置**：点击"新建配置文件"创建空配置

#### 2. 保存配置
根据加载方式的不同，有两种保存方式：
*   **保存回原文件**：直接覆盖原配置文件
*   **另存为**：下载为`config.json`文件，需手动替换原文件

### 注意事项
*   BGI安装在特定目录下可能无法手动选择文件，需拖拽导入
*   修改后请及时保存，未保存的修改在刷新页面后会丢失


## 五、手动编辑配置文件 (config.json)
...
[
  {
        ...
        "fightParam": {                       // 战斗参数配置
            "timeout": 240,                   // 战斗超时时间，单位为秒，默认240秒
            "strategyName": "根据队伍自动选择" // 战斗策略名，需与AutoFight目录下的策略文件名对应
        }
    }
]
