...
function initMappings() {
    ...
    for (i = 0; i < MAPPINGS.characters.length; i++) {
        entry = MAPPINGS.characters[i];
        CHARACTER_NAME_MAP[entry.names.zh] = entry.id;
        if (entry.c3 || entry.c5) {
            CHARACTER_CONST_BONUS[entry.id] = { c3: entry.c3 || null, c5: entry.c5 || null };
        }
    }
    ...
    for (i = 0; i < MAPPINGS.artifactSets.length; i++) {
        entry = MAPPINGS.artifactSets[i];
        ARTIFACT_SET_MAP[entry.names.zh] = entry.id;
        ARTIFACT_SET_MAX_RARITY[entry.id] = entry.rarity;
    }
    ...
    for (i = 0; i < NAME_OVERRIDES.length; i++) {
