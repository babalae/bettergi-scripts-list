...
export async function checkHolyRelicsKey(threshold=400) {
    ...
    if (textFind===null) {  // 如果未找到"圣遗物"文本
        ...
        // 点击圣遗物背包
        await findImgAndClick('assets/holyRelics.jpg')  // 通过图片点击进入圣遗物背包
    }
    ...
    const count = parseInt(strings[0]);  // 解析当前数量
    const total = parseInt(strings[1]);  // 解析总容量
    ...
    if (diff <= threshold) {  // 如果剩余空间小于等于阈值
        ...
        // 发送提醒消息
        await sendText(`背包圣遗物 剩余空间不足:${threshold},剩余:${diff},请前往清理!`, "背包圣遗物剩余空间检查")  // 发送提醒文本
    }
}