...
const archers1 = settings.archers1 || '';//多人路线角色不能正常切换，暂时放这里
const archers2 = settings.archers2 || '';
...
/**
        ...
        safeNormalMode: async function(params = {}) {
            ...
            /* ============== 换人前台函数 ============== */
    swapArcherShield: async function (archers1,archers2) {
        /* 0. 检测玩家数量 → 决定操作位 */
        await genshin.returnMainUi();
        const pc = await Multiplayer.detectPlayerCount();
        if (!pc || pc.status === false) {
        ...
        /* 1. 强制固定换？ */
        if (!archers1 && !archers2) {
            const archerIdx = count <= 1 ? 0 : 2;
            const shieldIdx = count <= 1 ? 1 : 3;
            
            // 检查是否已经是正确
            const isAlready = current[archerIdx] === archers1 && current[shieldIdx] === archers2;
            if (isAlready) {
                log.info(`[强制] ${count}人队伍 目标位已即位，跳过换人`);
                return 'ok';
            }
            
            // 检查是否错位
            const isWrongPosition = current[archerIdx] === archers2 && current[shieldIdx] === archers1;
            
            if (isWrongPosition) {
                log.info(`[强制] ${count}人队伍 检测到错位，进行中转互换`);
                ...
                finalPosition[archerIdx] = archers1;
                finalPosition[shieldIdx] = archers2;
                ...
                log.info(`[强制] ${count}人队伍 已完成错位修正`);
                return 'ok';
            }
            
            // 普通强制替换（既不是正确位置也不是错位情况）
            const position = [...current]; // 保持其他位置不变
            position[archerIdx] = archers1;
            position[shieldIdx] = archers2;
