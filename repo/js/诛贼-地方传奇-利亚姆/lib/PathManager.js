...
// 依赖：Utils.js, OCR.js, TaskManager
...
var PathManager = {
     ...
     */
    startProcessingTask: async function(folderGroups, accountName, taskDescription = '路径处理任务') {
        // 重置取消状态
        this.isCancelled = false;
        
        // 创建任务
        const taskWrapper = TaskManager.createPathingTask(
            taskDescription,
            async () => {
                return await this._processGroupsInternal(folderGroups, accountName, taskWrapper.taskId);
            },
            () => {
                // 清理函数
                log.info('清理路径处理任务资源...');
        ...
        this.currentTaskId = taskWrapper.taskId;
        
        try {
            log.info(`开始路径处理任务: ${taskDescription} (ID: ${this.currentTaskId})`);
            const result = await taskWrapper.execute();
            log.info(`路径处理任务完成: ${taskDescription}`);
            return {
                success: true,
            ...
            if (error.message.includes('Task was canceled')) {
                log.info(`路径处理任务被取消: ${taskDescription}`);
                return {
                    success: false,
                    cancelled: true,
                    taskId: this.currentTaskId,
                ...
                log.error(`路径处理任务失败: ${error.message}`);
                return {
                    success: false,
                    taskId: this.currentTaskId,
                    error: error.message
     ...
     */
    stopProcessingTask: function(reason = '用户请求') {
        if (this.currentTaskId && TaskManager.isTaskActive(this.currentTaskId)) {
            TaskManager.cancelTask(this.currentTaskId, reason);
            this.isCancelled = true;
            return true;
        }
        return false;
     ...
     */
    shouldInterrupt: function(taskId = null) {
        const id = taskId || this.currentTaskId;
        if (id) {
            return !TaskManager.isTaskActive(id);
        }
        return TaskManager.shouldInterrupt() || this.isCancelled;
    },
     ...
     */
    _processGroupsInternal: async function(folderGroups, accountName, taskId) {
        // 1. 加载OCR过滤名单
        ...
        for (const [groupName, pathList] of Object.entries(folderGroups)) {
            // 检查是否应该中断
            if (this.shouldInterrupt(taskId)) {
                log.info('任务被中断，退出分组循环');
                return;
            }
            ...
            for (const pathing of pathList) {
                // 检查是否应该中断
                if (this.shouldInterrupt(taskId)) {
                    log.info('任务被中断，退出路线循环');
                    return;
                }
                
                try {
                    await TaskManager.safeExecute(async () => {
                        // 5.1 初始化路线数据
                    ...
                    }, taskId, `执行路线 ${pathing.fileName}`);
                } catch (error) {
                    if (error.message.includes('Task was canceled')) {
                        log.info('路线执行被取消');
                        throw error; // 重新抛出，让外层处理
                    }
                    log.error(`路线执行异常: ${error.message}`);
                    // 继续处理下一条路线
                }
            }
        }
        
        log.info(`所有路线处理完成，共处理 ${handledRoutes}/${totalRoutes} 条路线`);
        return {
            totalRoutes,
            handledRoutes,
            success: true
    ...
    processGroups: async function(folderGroups, accountName) {
        return this.startProcessingTask(folderGroups, accountName, '路径分组处理');
    },
    ...
    runPath: async function(pathFilePath, whitelistKeywords, blacklistKeywords) {
        return this._runPathInternal(pathFilePath, whitelistKeywords, blacklistKeywords, null);
    },
     ...
     * @returns {Promise<void>}
     */
    runPathWithTaskId: async function(pathFilePath, whitelistKeywords, blacklistKeywords, taskId) {
        return this._runPathInternal(pathFilePath, whitelistKeywords, blacklistKeywords, taskId);
    },
    
    /**
     * 内部路径执行逻辑
     */
    _runPathInternal: async function(pathFilePath, whitelistKeywords, blacklistKeywords, taskId) {
        // ------------------ 状态变量 ------------------
        ...
        function checkCancellation() {
            if (taskId && !TaskManager.isTaskActive(taskId)) {
        ...
        async function isMainUI() {
            const MAIN_UI_IMAGE = `${PathManager.ASSETS_FOLDER}/MainUI.png`;
            const SCAN_REGION = { x: 0, y: 0, width: 150, height: 150 };
            const MAX_ATTEMPTS = 2;
            
            let attempts = 0;
            while (attempts < MAX_ATTEMPTS && !checkCancellation()) {
                try {
        ...
        async function performOcrAndInteract() {
            ...
            // OCR配置常量
            const F_ICON_PATH = `${PathManager.ASSETS_FOLDER}/F_Dialogue.png`;
            const OCR_REGION = {
            ...
            async function performOcr(keywords, yRange, timeout = 200) {
                const startTime = Date.now();
                while (Date.now() - startTime < timeout && !checkCancellation()) {
                    try {
            ...
            // ========================== OCR主循环 ==========================
            while (!state.completed && !checkCancellation()) {
                try {
                    // 1. 查找F交互图标
                    ...
                    if (!fIcon) {
                        ...
                        if (state.atMainUi && !checkCancellation()) {
                            // 在主界面时向下滚动寻找物品
                            try {
                                await keyMouseScript.runFile(`${PathManager.ASSETS_FOLDER}/滚轮下翻.json`);
                            } catch (error) {
                                if (error.message && error.message.includes('was canceled')) {
                                    state.cancelRequested = true;
                                    break;
                    ...
                    if (!foundValidItem) {
                        ...
                        if (now - lastMoveDown > timeMoveUp) {
                            ...
                            // 向上滚动（返回确认）
                            try {
                                await keyMouseScript.runFile(`${PathManager.ASSETS_FOLDER}/滚轮上翻.json`);
                            } catch (error) {
                                if (error.message && error.message.includes('was canceled')) {
                                    state.cancelRequested = true;
                                    break;
