...
//  * ========================================================= */
        const option = settings.option || '推荐-换人';
        const charAvail = settings.liam_char_availability || "我都有";
...
/**
    ...
    swapArcherShield: async function () {
        ...
        const forceFixedSwap = charAvail === "我都有";
        /* 1. 强制固定换？ */
        if (forceFixedSwap) {
            ...
            // 检查是否已经是正确的提纳里和钟离
            const isAlready = current[archerIdx] === '提纳里' && current[shieldIdx] === '钟离';
            if (isAlready) {
                log.info(`[强制] ${count}人队伍 目标位已是提纳里+钟离，跳过换人`);
                return 'ok';
            }
            
            // 检查是否错位（提纳里在盾位，钟离在弓箭手位）
            const isWrongPosition = current[archerIdx] === '钟离' && current[shieldIdx] === '提纳里';
            
            if (isWrongPosition) {
                log.info(`[强制] ${count}人队伍 检测到错位：弓箭手位是钟离，盾位是提纳里，进行中转互换`);
                
                // 第一步：先把两个位置换成临时角色（安柏和钟离）
                const tempPosition = [...current];
                tempPosition[archerIdx] = '安柏';
                tempPosition[shieldIdx] = '钟离';
                ...
                finalPosition[archerIdx] = '提纳里';
                finalPosition[shieldIdx] = '钟离';
                ...
                log.info(`[强制] ${count}人队伍 已完成错位修正 提纳里→${archerIdx + 1}号位 钟离→${shieldIdx + 1}号位`);
                return 'ok';
            }
            
            // 普通强制替换（既不是正确位置也不是错位情况）
            const position = [...current]; // 保持其他位置不变
            position[archerIdx] = '提纳里';
            position[shieldIdx] = '钟离';
            ...
            log.info(`[强制] ${count}人队伍 已固定换 提纳里→${archerIdx + 1}号位 钟离→${shieldIdx + 1}号位`);
            return 'ok';
        }

        const forceFixedSwap1 = charAvail !== "没有提纳里";
        if (forceFixedSwap1) {
        /* 没有角色的情况：不符合 → 直接用弓箭手换成安柏，盾位换成女仆 */
        const positionFinal1 = [...current];
        positionFinal1[archerIdx] = '安柏';
        positionFinal1[shieldIdx] = '诺艾尔';

        // 将数组转换为对象格式
        await this._normalMode({
            position1: positionFinal1[0],
            position2: positionFinal1[1],
            position3: positionFinal1[2],
            position4: positionFinal1[3]
        });
        log.info(`智能换人：已换为 ${archerIdx + 1}号位安柏、${shieldIdx + 1}号位女仆`);
        return 'ok';
        ...
        /* 2. 池子匹配 + 是否已符合， */
        const archers = [
            '温迪', '甘雨', '雅珂达', '宵宫', '提纳里', '五郎', '安柏',
            '珐露珊', '恰斯卡', '林尼', '柯莱', '菲谢尔', '九条裟罗',
            '欧洛伦', '阿洛伊'
        ];
        const shields = [
            '钟离', '白术', '茜特菈莉', '诺艾尔', '迪奥娜', '莱依拉',
            '绮良良', '辛焱', '托马', '北斗'
        ];

        const archerIdx = count <= 1 ? 0 : 2;   // 1弓 or 3弓
        const shieldIdx = count <= 1 ? 1 : 3;   // 2盾 or 4盾
        ...
        // if (isOrderOk) {
        //     log.info(`智能换人：${archerIdx + 1}号位已是弓箭手、${shieldIdx + 1}号位已是盾，无需操作`);
        //     return 'ok';
        // }
        ...
        // 如果是两人互换的情况（弓箭手在盾位，盾在弓箭手位）
        if (archerInShieldPos && shieldInArcherPos) {
            log.info(`检测到两人互换：弓箭手"${current[shieldIdx]}"在盾位，盾"${current[archerIdx]}"在弓箭手位`);
            ...
            // finalPosition[shieldIdx] = current[archerIdx];  // 原弓箭手位的盾放到盾位，换钟离就不变
        ...
        /* 4. 普通情况：不符合 → 直接用弓箭手换成安柏，盾位换成钟离 */
        const positionFinal = [...current];
        positionFinal[archerIdx] = '安柏';
        positionFinal[shieldIdx] = '钟离';
        ...
        log.info(`智能换人：已换为 ${archerIdx + 1}号位安柏、${shieldIdx + 1}号位钟离`);
        return 'ok';
    }
