...
var core = {
     ...
     * ------------------------------------------------------ */
    mora: async function () {
        const CharacterMenuRo = RecognitionObject.TemplateMatch(file.ReadImageMatSync("assets/Laim/Recognition/Friends Button.png"));
    ...
    executeMainProcess: async function () {
        ...
        await this.recognizeOnePopup();
            ...
            if ((delta >= 2000 )&& pick === "我全都要！") {    await MobFallAttack.pickUp();    }
                await genshin.tpToStatueOfTheSeven();
        await this.recognizeOnePopup();
            return delta >= 2000;
        } catch (err) {
...
recognizeOnePopup: async function() {
    ...
    const iconFiles = `lib/Laim/assets/a.png`;
