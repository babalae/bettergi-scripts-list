...
(async function () {
    ...
    async function taskSetupPermission() {
        ...
        var mode = PERMISSION_MODES[domainName];
        if (!mode) {
    ...
    async function taskCraftResin() {
        ...
        var region = settings.craftRegion || "枫丹";
        try {
    ...
    async function taskClaimDailyAndExpedition() {
        ...
        var region = settings.guildRegion || "枫丹";
        try {
    ...
    async function main() {
        ...
        // 解析配置
        var retryCount = parseInt(settings.globalRetryCount, 10) || 3;
        var timeoutSec = parseInt(settings.globalTimeoutSec, 10) || 60;
        var enableCraftResin = settings.enableCraftResin !== false;
        var enableBattlePass = settings.enableBattlePass !== false;
        var enableMail = settings.enableMail !== false;
